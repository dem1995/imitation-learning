"""
A domain-specific language for interfacing with Can't Stop. Contains a CFG
for generating random clauses for use by agents.
"""
...
class DSL:
    ...
    def RandomRule():
        """
        Generates a random if-clause using the DSL's context-free grammar
        """
        rule = '$B$'
        while any(nonterminal in rule for nonterminal in DSL._grammar.keys()):
            nonterminal = next(nonterminal for nonterminal in DSL._grammar.keys() if nonterminal in rule)
            replacements = DSL._grammar[nonterminal]
            replacement = random.choice(replacements)
            rule = rule.replace(nonterminal, replacement, 1)
        return rule
